...
RSpec.describe AdaptersTrackingInformation::FedexTrackingInformation, :type => :adapter do
    ...
    context 'En el caso de que fedex nos retorne respuesta exitosa' do
        before do
            allow_any_instance_of(Fedex::Request::TrackingInformation).to receive(:process_request).and_return([OpenStruct.new(tracking_number: '123456789', status_code: status_code)])
        end
    ...
    context 'En el caso de que fedex no encuentre el paquete' do
        before do
            allow_any_instance_of(Fedex::Request::TrackingInformation).to receive(:process_request).and_raise(Fedex::RateError, 'This tracking number cannot be found. Please check the number or contact the sender.')
        end

        context 'Y tengamos registro de ese paquete' do
            let!(:tracking_information_in_db) {TrackingInformation.create(tracking_number: '123456789', carrier: 'FEDEX', status: 'DELIVERED')}
    ...
    context 'En el caso de que devuelva un error de request fedex' do
        before do
            allow_any_instance_of(Fedex::Request::TrackingInformation).to receive(:process_request).and_raise(Fedex::RateError, 'ERROR')
        end
