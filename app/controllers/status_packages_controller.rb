class StatusPackagesController < ApplicationController
    def GetTrackingInformationInFedex  
        if (tracking_params[:tracking_number].present? && tracking_params[:callback_url].present?)
            TrackingInformationFedexWorker.perform_async(tracking_params[:tracking_number], tracking_params[:callback_url])
            render :ok, json: { message: 'Se esta solicitando el tracking a Fedex, ni bien tengamos novedades las comunicaremos a: ' + tracking_params[:callback_url]}
        else
